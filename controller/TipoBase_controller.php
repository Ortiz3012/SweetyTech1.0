...
include_once '../../model/TipoBase_model.php';

class TipoBaseController extends conexion
{
  public function Listar()
  {
    $datosTipoBase=array();
     $consultar="SELECT * FROM tbl_tipo_base ORDER BY Id_Tipo_Base";
     ...
     try {
          ...
          // $resultado->execute();
           ...
           foreach ($resultado->fetchALL(PDO::FETCH_OBJ) as $dato) {
               ...
               $TipoBase->__SET('Id_Tipo_Base',$dato->Id_Tipo_Base);
               $TipoBase->__SET('Nombre',$dato->Nombre);
               $datosTipoBase[]=$TipoBase;
           }

         return $datosTipoBase; 
...
public function eliminar($Id_Tipo_Base)
{
  ...
  try{
    ...
    $this->conexion->prepare($borrar)->execute(array($Id_Tipo_Base));
    return true;

   }catch(\Exeption $error){
    echo 'error al Eliminar los datos'.$error->getMessage();
